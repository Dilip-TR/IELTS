...
.state('users.create', {
    ...
    .state('writingreview.create', {
        url: "/create/:userid?testid:totalid",
        views: {
            "@": {
        ...
        .state('subcategory.create', {
            ...
            views: {
                "@": {
                    ...
                    controller: 'subcategoryCreateCtrl'
                }
            },
...
angular.module('inspinia').service('awareviewService', ['$q', '$http', 'configurationService', function($q, $http, configurationService) {
    ...
    this.getawsList = function(name) {
        var D = $q.defer();
        var data = {};
        $http.get(configurationService.baseUrl() + '/Test/aws_cmsque_list/'+name).success(function(data) {
            D.resolve(data);
        });
        return D.promise;
    }
    ...
    // http://localhost/services/softprep/api/Test/awa_update_test
    http://localhost/services/softprep/api/user/loginget/51
    this.updateawaQua = function(data) {
        var D = $q.defer()
        $http.post(configurationService.baseUrl() + '/Test/awa_update_test', data).success(function(data) {
            D.resolve(data);
        }).error(function(data) {
            D.resolve(data);
        });
...
angular.module('inspinia')
    .controller('writingreviewListCtrl', ["$scope", "$state", "$rootScope", "$interval", "photogalleryService", "awareviewService", "$cookieStore", "$stateParams", "$timeout", "subcategoryService", function($scope, $state, $rootScope, $interval, photogalleryService,awareviewService, $cookieStore, $stateParams, $timeout, subcategoryService) {
        var vm = this;
        ...
        $scope.changeCategory=function(){
            ...
            if($scope.post.catVal){
                awareviewService.getawsList($scope.post.catVal).then(function(data) {
                    console.log(data);
                    if (data) {
                        $scope.catList = data;
                    } else {
        ...
        $scope.review = function(id,testid) {
           $state.go('awareview.create',{'userid':id,'testid':testid,'totalid':id+testid});
        }
...
angular.module('inspinia').controller('writingreviewCreateCtrl', ["$scope", "$interval", "$cookieStore", "$state", "writingreviewService", "$rootScope", "vediogalleryService", "$stateParams", "FileUploader", "configurationService", "TestService", "$timeout", "subcategoryService", "$filter", function($scope,$interval,$cookieStore, $state,writingreviewService,$rootScope,vediogalleryService,$stateParams, FileUploader, configurationService, TestService,$timeout ,subcategoryService, $filter) {
    ...
    $scope.scores = [{ name: 1.0 ,id: 1.0 }, { name: 2.0, id:2.0 },{ name: 3.0, id: 3.0 }];
        ...
        writingreviewService.getawsQueList($scope.testid).then(function(data) {
           // console.log(data);
            if (data) {
                ...
                var getType7Que=_.filter(data.questions,function(x){
                    return x.question_type_id === "7" ;
                });
                _.map(getType7Que,function(x){
                    return x.options[0].sugestion = "" ;
                });
                _.map(getType7Que,function(x){
                    return x.btnChecked = 0 ;
                });
                $scope.alltestList = getType7Que;
             }else{
                $scope.errorLogin = "Login failed! Please Check Username And Password";
                //console.log('Please Check Username And Password');    
            }
    ...
    $scope.btnSave=function(){
        $scope.errorMsg="" ;
        var notCorrected=_.filter($scope.alltestList,function(x){
            return x.btnChecked === 0 ;
        });
        console.log(notCorrected) ;
       // notCorrected=0;
        if(notCorrected != 0){
            //alert("Please Evaluate all Questions");
            //"api/Test/save_test"
            $scope.errorMsg="Please Evaluate all Questions";
        }else{
            angular.forEach($scope.alltestList,function(val,key){
                  console.log(val);
                  var getInd=_.findIndex($scope.totalObj.questions,function(x){
                         return x.trackid === val.trackid ;
                     });
                     console.log($scope.totalObj.questions[getInd]);
                     $scope.totalObj.questions[getInd] =val ;
            });
            console.log($scope.totalObj);
        }
        $scope.totalObj.awsstatus="completed";
      var sendObj={
        "useId":$stateParams.userid,
        "testId":$stateParams.testid,
        "phoneNo" :$scope.catList.phonenumber,
        ...
        "graphdata" : {
            ...
            var getScr= _.filter($scope.totalObj.questions,function(x){
                      return x.category  ===  "awa" ;
            });
            var totalCountgetScr = 0 ;
            angular.forEach(getScr,function(val ,key){
                    console.log(val.options[0].score) ;
                    totalCountgetScr= totalCountgetScr + val.options[0].score ;
            });
           // sendObj.graphdata
          // console.log($scope.catList.phonenumber);
         sendObj.graphdata.awsscore = totalCountgetScr ;    
         var getGreScore={"quant":0 , "verbal":0, "greScore":0 };
         for(var i=2; i<6 ; i++){
            console.log(i,$scope.totalObj.sections[i]) ;
            if($scope.totalObj.sections[i].category == "quant"){
                getGreScore.quant = getGreScore.quant+$scope.totalObj.sections[i].result ; 
            }else{
                getGreScore.verbal = getGreScore.verbal+$scope.totalObj.sections[i].result ; 
            }
        }
        getGreScore.verbal=getGreScore.verbal + 130 ;
        getGreScore.quant = getGreScore.quant + 130 ;
        getGreScore.greScore =  getGreScore.verbal + getGreScore.quant ;
        $scope.totalGreScor=getGreScore ;
        sendObj.graphdata.verbal = getGreScore.verbal ;
        sendObj.graphdata.quant =  getGreScore.quant ;
        sendObj.graphdata.grescore = getGreScore.greScore ;
        console.log(sendObj);
        writingreviewService.updateawaQua(sendObj).then(function(res) {
              console.log(res);
              $cookieStore.put("checkIsMiddle1",$scope.totalObj.awsstatus); 
        });
        $state.go('awareview');
    }
...
angular.module('inspinia').controller('vediogalleryCreateCtrl', ["$scope", "$state", "$rootScope", "$stateParams", "FileUploader", "configurationService", "TestService", "$timeout", "vediogalleryService", "$filter", function($scope, $state, $rootScope, $stateParams, FileUploader, configurationService, TestService,$timeout, vediogalleryService, $filter) {
...
angular.module('inspinia').controller('quationsCreateCtrl', ["$scope", "photogalleryService", "$state", "$rootScope", "$stateParams", "FileUploader", "configurationService", "$timeout", "quationsService", "categoryService", "$filter", function($scope, photogalleryService, $state, $rootScope, $stateParams, FileUploader, configurationService, $timeout, quationsService, categoryService, $filter) {
    ...
    $scope.post={
        ...
        $scope.addOption=function(){
            $scope.post.options.push({"name": "", "is_answer" : [""] ,"user_aws":"" });
        }
        $scope.removeOption=function(index){
            console.log(index);
    ...
    //     "is_answer": [],
    // }];
    // // $scope.post.options[ $scope.act].is_answer.push({
    // //     "question_id": "",
    ...
    // }
    // $scope.addOption1 = function(index) {
         
    //    $scope.post.options.is_answer.push({
    //        "is_answer":"",
    //    }); 
...
$templateCache.put("app/paragraph/xxx.html","<table border=\"1\" cellpadding=\"5\" cellspacing=\"0\" style=\"width: 406px; height: 191px;\"><tbody><tr><td style=\"background-color: #dddddd;\">Name</td><td style=\"background-color: #dddddd;\">William Glass</td></tr><tr><td>Email</td><td><span class=\"number\">1</span> <input type=\"text\" class=\"inputstyle num_1\" ng-model=\"options.user_aws\"></td></tr><tr><td>Date</td><td>26th February 2007</td></tr><tr><td>Occupation</td><td><span class=\"number\">2</span> <input type=\"text\" class=\"inputstyle num_2\" ng-model=\"options.user_aws\"></td></tr><tr><td>Age</td><td>29</td></tr><tr><td>Marital Status</td><td><span class=\"number\">3</span> <input type=\"text\" class=\"inputstyle num_3\" ng-model=\"options.user_aws\"></td></tr><tr><td>Children</td><td>no</td></tr><tr><td>Salary</td><td>£ <span class=\"number\">4</span> <input type=\"text\" class=\"inputstyle num_4\" ng-model=\"options.user_aws\"> thousand/ year&nbsp;</td></tr></tbody></table>");
...
$templateCache.put("app/writingreview/create.html","<div class=\"wrapper wrapper-content animated fadeIn\"><div class=\"row\"><div class=\"col-lg-12 ibox-content\"><ul class=\"nav nav-tabs\"></ul><div class=\"tab-content\"><div class=\"tab-pane active\" id=\"tab_a\"><div class=\"col-lg-12\"><div class=\"form-group\"><label style=\"color: blueviolet;\">Question</label><p ng-bind-html=\"alltestList[currentQue].name\"></p></div></div><div class=\"col-lg-12\"><div class=\"form-group\"><label style=\"color: blueviolet;\">UserAnswer</label><p ng-bind-html=\"alltestList[currentQue].options[0].user_aws\"></p></div></div><div class=\"col-lg-12\"><label style=\"color: blueviolet;\">Sugetion</label><text-angular ng-model=\"alltestList[currentQue].options[0].sugestion\" focus-me=\"shouldBeOpen\"></text-angular><button type=\"button\" class=\"btn btn-info btn-lg\" data-toggle=\"modal\" data-target=\"#myModal\">Browse</button></div><div class=\"col-lg-12\"><label style=\"color: blueviolet;\">Score</label><select ng-model=\"alltestList[currentQue].options[0].score\" ng-options=\"score.name as score.name for score in scores\"><option value=\"\">Select Score</option></select><div><span style=\"color:red\">{{errorMsg}}</span></div><div class=\"form-group\"><button type=\"reset\" ng-click=\"btnNext()\" class=\"btn btn-warning\" ng-if=\"alltestList.length-1 !=currentQue\">Load Next Question</button> <button type=\"reset\" ng-click=\"btnPre()\" class=\"btn btn-info\" ng-if=\"currentQue != 0\">Load Previous Question</button><button type=\"reset\" ng-click=\"btnSave()\" class=\"btn btn-danger\">Save review data</button></div></div></div></div></div></div></div>");
$templateCache.put("app/writingreview/list.html","<div class=\"wrapper wrapper-content animated fadeIn\"><div class=\"row\"><div class=\"col-sm-12\"><div class=\"ibox\"><div class=\"ibox-title\"><span class=\"pull-right\"><div class=\"form-group\"><select ng-model=\"post.catVal\" ng-options=\"cat.name as cat.name for cat in categories\"><option value=\"\" disabled=\"\" hidden=\"\" selected=\"\">Select Branch</option></select><button type=\"button\" class=\"btn btn-primary btn-xs\" ng-click=\"changeCategory()\">Go</button></div></span></div><div class=\"ibox-content\" style=\"border:none;\"><div class=\"clients-list\"><div class=\"full-height-scroll\"><div class=\"table-responsive\"><table class=\"ordersView\" datatable=\"ng\" dt-options=\"dtOptions\"><thead><tr><th>Id</th><th>Userid</th><th>AwsQueStatus</th><th>Branch</th><th>Testtype</th><th>Action</th></tr></thead><tbody><tr ng-repeat=\"cat in catList\"><td>{{$index+1}}</td><td>{{cat.userid}}</td><td>{{cat.awsstatus |capitalize}}</td><td>{{cat.branch}}</td><td>{{cat.testtype }}</td><td><button type=\"button\" ng-click=\"review(cat.userid,cat.testId)\" ng-disabled=\"false\" class=\"btn btn-danger btn-xs\">Review</button></td></tr></tbody></table></div></div></div></div><br></div></div></div></div>");
...
$templateCache.put("components/common/navigation.html","<nav class=\"navbar-default navbar-static-side\" role=\"navigation\"><div class=\"sidebar-collapse\"><ul side-navigation=\"\" class=\"nav metismenu\" id=\"side-menu\"><li class=\"nav-header\"><div class=\"dropdown profile-element\" dropdown=\"\"><a class=\"dropdown-toggle\" dropdown-toggle=\"\" href=\"\"><span class=\"clear\"><span class=\"block m-t-xs\"><img class=\"logo\" src=\"assets/images/logo.png\" alt=\"Logo\"></span></span></a><ul class=\"dropdown-menu animated fadeInRight m-t-xs\"><li><a href=\"\">Logout</a></li></ul></div><div style=\"font-size: 16px;\" class=\"logo-element\">IELTS</div></li><li ng-class=\"{active: $state.includes(\'users\')}\"><a><i class=\"fa fa-user\"></i> <span class=\"nav-label\">Users</span> <span class=\"fa arrow\"></span></a><ul class=\"nav nav-second-level collapse\" ng-class=\"{in: $state.includes(\'users\')}\"><li ui-sref-active=\"\"><a ui-sref=\"users\">List</a></li></ul></li><li ng-class=\"{active: $state.includes(\'paragraph\')}\"><a><i class=\"fa fa-book\"></i> <span class=\"nav-label\">Paragrapgh</span> <span class=\"fa arrow\"></span></a><ul class=\"nav nav-second-level collapse\" ng-class=\"{in: $state.includes(\'paragraph\')}\"><li ui-sref-active=\"\"><a ui-sref=\"paragraph\">Paragraph</a></li></ul></li><li ng-class=\"{active: $state.includes(\'photogallery\')}\" ng-if=\"userDetails.id !=2\"><a><i class=\"fa fa-image\"></i> <span class=\"nav-label\">FileUpload</span> <span class=\"fa arrow\"></span></a><ul class=\"nav nav-second-level collapse\" ng-class=\"{in: $state.includes(\'photogallery\')}\"><li ui-sref-active=\"\"><a ui-sref=\"photogallery\">List</a></li></ul></li><li ng-class=\"{active: $state.includes(\'test\')}\" ng-if=\"userDetails.id !=2\"><a><i class=\"fa fa-bicycle\"></i> <span class=\"nav-label\">Test</span> <span class=\"fa arrow\"></span></a><ul class=\"nav nav-second-level collapse\" ng-class=\"{in: $state.includes(\'test\')}\"><li ui-sref-active=\"\"><a ui-sref=\"test\">List</a></li></ul></li><li ng-class=\"{active: $state.includes(\'writingreview\')}\" ng-if=\"userDetails.id !=2\"><a><i class=\"fa fa-video-camera\"></i> <span class=\"nav-label\">WritingReview</span> <span class=\"fa arrow\"></span></a><ul class=\"nav nav-second-level collapse\" ng-class=\"{in: $state.includes(\'writingreview\')}\"><li ui-sref-active=\"\"><a ui-sref=\"writingreview\">List</a></li></ul></li></ul></div></nav>");

 ...
 */
class Pragoption extends REST_Controller {
    ...
    public function create_paragraph_post()
    {
            $input_data = json_decode(trim(file_get_contents('php://input')), true);

            $this->load->model('Praoption_model');
            $test=$this->Praoption_model->new_par($input_data);
             try { 
                if($test) {
                   $finall=array(array("response"=> "success" , "mesg"=> "" ,"trackid" => $test)) ;
        ...
        public function create_paragraph_que_post()
    {
            $input_data = json_decode(trim(file_get_contents('php://input')), true);

            $this->load->model('Praoption_model');
            $test=$this->Praoption_model->new_par_que($input_data);
             try { 
                if(empty($test)) {
     ...
     public function create_fulllength_post()
    {
            $input_data = json_decode(trim(file_get_contents('php://input')), true);

            $this->load->model('Praoption_model');
            $test=$this->Praoption_model->new_fulllength($input_data);
             try { 
                if(empty($test)) {
                     $finall=array(array("response"=> "success" , "mesg"=> "")) ;
                     $this->set_response( $finall, REST_Controller::HTTP_OK); 
    ...
    public function test_all_list_get()
      {
              // $input_data = json_decode(trim(file_get_contents('php://input')), true);
              $this->load->model('Praoption_model');
              $commission_one=$this->Praoption_model->test_list() ;
              try { 
        ...
        public function list_paragraph_get()
            {
                   // $name=$this->uri->segment('4');
                    $this->load->model('Praoption_model');
                    $commission_one=$this->Praoption_model->getlist_by_par() ;
             ...
             public function list_fullength_tests_get()
            {
                    $input_data = (int)$this->uri->segment('4');
                    $this->load->model('Praoption_model');
                    $commission_one=$this->Praoption_model->getlist_by_fullLength($input_data) ;
            ...
            public function list_paragraph_ques_get()
            {
                   $id=$this->uri->segment('4');
                    $this->load->model('Praoption_model');
                    $commission_one=$this->Praoption_model->getlist_by_par_que($id) ;
                    try { 
                        if(empty($commission_one)) {
                            $this->set_response([], REST_Controller::HTTP_OK);
                        }else{
              ...
              public function fulllength_test_get()
            {


                    $id=$this->uri->segment('4');
                    $this->load->model('Praoption_model');
                    $commission_one=$this->Praoption_model->getfulllength_test_by_id($id) ;
                    try { 
                        if(empty($commission_one)) {
                            $this->set_response([], REST_Controller::HTTP_OK);
                        }else{
    ...
    public function update_parabyque_post()

    {
        ...
        // exit;
            $input_data = json_decode(trim(file_get_contents('php://input')), true);
            $this->load->model('Praoption_model');
            $test=$this->Praoption_model->update_par_que($input_data);
             try { 
                if(empty($test)) {
                     $finall=array(array("response"=> "success" , "mesg"=> "") ) ;
            ...
            public function list_paragraph_bytrackid_get(){
                $tid=(int)$this->uri->segment('4');
                // print_r($tid);
                // exit;
                // Users from a data store e.g. database
                $this->load->model('Praoption_model');
                $users= $this->Praoption_model->pargraphById($tid) ;
                try { 
                    if(empty($users)) {
                        $this->set_response([], REST_Controller::HTTP_OK);
                    }else{
             ...
             public function list_paragraph_quesoptnsByid_get(){
                $qid=(int)$this->uri->segment('4');
                // print_r($qid);
                // exit;
                // Users from a data store e.g. database
                $this->load->model('Praoption_model');
                $users= $this->Praoption_model->quaoptnById($qid) ;
                try { 
                    if(empty($users)) {
                        $this->set_response([], REST_Controller::HTTP_OK);
                    }else{
            ...
            public function categoryget_by_cat_get()
            {
                    $name=$this->uri->segment('4');
                    $this->load->model('Praoption_model');
                    $commission_one=$this->Queoption_model->getlist_by_name($name) ;
                 ...
                 public function get_paragraph_get()
            {
                    $name=$this->uri->segment('4');

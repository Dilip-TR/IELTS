...
class Gallery extends REST_Controller {
    ...
    public function list_get()
    {
        ...
        $users= $this->gallery_model->listData() ;
       ...
       public function deletegallery_delete()
        {
            $id=(int)$this->uri->segment('4');
            // Users from a data store e.g. database
            $this->load->model('gallery_model');
            $users= $this->gallery_model->galleryrow_delete($id) ;
            try { 
                if(empty($users)) {
                    $this->set_response([], REST_Controller::HTTP_OK);
                }else{
    ...
    public function vediolist_get()
    {
        ...
        // Users from a data store e.g. database
        $this->load->model('gallery_model');
        $users= $this->gallery_model->vedioListData() ;
      ...
      public function clientlist_get()
    {
        ...
        // Users from a data store e.g. database
        $this->load->model('gallery_model');
        $users= $this->gallery_model->clientlistData() ;
        ...
        public function bannerlist_get()
    {
        ...
        // Users from a data store e.g. database
        $this->load->model('gallery_model');
        $users= $this->gallery_model->bannerlistData() ;
        try { 
            if(empty($users)) {
                $this->set_response([], REST_Controller::HTTP_OK);
            }else{
    ...
    //     $rows= $this->industries_model->listData() ;
    //     try { 
    //         if(empty($rows)) {
    //             $this->set_response([], REST_Controller::HTTP_OK);
    //         }else{
    ...
    public function add_post()
    {
          ...
          //  $dd= parent::uriChange($input_data['name']);
           ...
           // $input_data['uri'] =$dd ; 
            ...
            $data=$this->gallery_model->insert_entry($input_data);
